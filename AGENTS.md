...
- **代码变动**：改完后端要考虑前端适配，改完前端要考虑后端适配，不能只顾一端改完就结束。
- **部署验证**：不论修改前端、后端还是网关代码，都必须使用 Docker Compose 重新部署服务并验证功能正常。

### Docker Compose 验证

- 改完前端和后端、网关代码后必须用 Docker Compose 构建重启验证功能，确保所有服务都能正常运行
...
| **auth**       | Microsoft 账号认证（MSAL设备码流程） | `src/agent/auth/`       |
| **automation** | 自动化步骤（登录、串流、解码、游戏）        | `src/agent/automation/` |
| **core**       | 中央管理、配置、日志、更新管理               | `src/agent/core/`       |
| **game**       | 游戏账号管理                        | `src/agent/game/`       |
| **input**      | 输入控制器（模拟按键等）                  | `src/agent/input/`      |
| **scene**      | 场景检测                          | `src/agent/scene/`      |
| **task**       | 任务调度、任务上下文管理                  | `src/agent/task/`       |
| **vision**     | 视觉识别、帧捕获、模板匹配                 | `src/agent/vision/`     |
| **windows**    | Windows 窗口管理、任务窗口管理            | `src/agent/windows/`    |
...
9. **代码编译检查清单**
   - ✅ **方法重复定义检查**：同一类中不允许存在方法名和参数类型完全相同的方法（非重载）
   - ✅ **导入语句检查**：使用的所有类、枚举、接口必须在文件顶部正确导入
   - ✅ **依赖注入检查**：使用 `@RequiredArgsConstructor` 注入的依赖必须在类中声明为 `final` 字段
   - ✅ **枚举引用检查**：使用枚举值前必须确认该枚举值存在于枚举定义中
   - ✅ **异常构造检查**：`BusinessException` 构造函数必须传入 `ResultCode` 枚举参数
...
#### 8. 自动化流程设计
- **步骤分离原则**：将复杂流程分解为独立步骤（登录、连接、解码、游戏）
...
#### 9. 模块化设计
- 每个模块职责单一
- **automation/**：仅包含自动化步骤文件（step1-step4）
- **task/**：任务调度、任务上下文管理
- **windows/**：窗口管理、任务窗口管理器
- **api/**：通信接口、平台 API 客户端
- 使用工厂模式创建专用日志记录器
- 避免模块间循环依赖
...
#### 13. 认证方案（MSAL设备码认证）
- **认证流程**：
  1. 首次登录：使用设备码认证，用户访问 `https://login.microsoft.com/device` 输入代码
  2. 获取 Refresh Token 并持久化存储
  3. 后续登录：自动使用 Refresh Token 刷新，无需用户干预
- **Token 存储**：
  - 文件路径：`tokens/refresh_tokens.json`
  - 格式：`{"email": "refresh_token", ...}`
  - 多账号支持：支持多个账号的 Token 独立存储
- **存储规则**：
  - `tokens/` 目录不应提交到代码仓库
  - 打包时不包含 `tokens/` 目录
  - 运行时自动创建并管理
...
#### 14. 调用链规范
- **消息接收**：WebSocket → WSClient → TaskExecutor
- **任务执行**：TaskExecutor → AgentAutomationTask → 步骤执行
- **步骤调用**：step1 → step2 → step3 → step4（顺序执行）
- **进度上报**：每个步骤通过 PlatformApiClient 主动上报到平台

#### 15. 双向通信
- **Agent → 平台**：连接建立、心跳保活、进度上报、结果上报、比赛数据上报
- **平台 → Agent**：任务下发、心跳确认、控制指令（取消/暂停/恢复）
...
| heartbeat\_ack | 平台 → Agent | 心跳确认    |
| task\_ack      | Agent → 平台 | 任务接收确认  |
