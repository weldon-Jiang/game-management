...
test.describe('导航和布局', () => {
  ...
  test('应该显示侧边栏', async ({ page }) => {
    await expect(page.locator('.aside, .sidebar, aside, [class*="sidebar"]').first()).toBeVisible()
  })

  test('侧边栏应该包含导航菜单', async ({ page }) => {
    await expect(page.getByRole('menuitem', { name: 'Agent管理' })).toBeVisible()
  })
  ...
  test('应该显示面包屑导航', async ({ page }) => {
    ...
    const breadcrumbs = page.locator('.el-breadcrumb')
