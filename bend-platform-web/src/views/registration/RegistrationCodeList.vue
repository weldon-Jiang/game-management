  ...
  <div class="page-container">
    <div class="page-header">
      <div class="header-left">
        <h2>分控注册码</h2>
        <span class="header-desc">为商户签发分控平台安装注册码（BEND-INSTALL-*），安装时一次性使用</span>
      </div>
    ...
    <div class="content-card">
      ...
      <div class="table-container">
        ...
        <el-table-column prop="merchantName" label="目标商户" min-width="120" align="center">
          <template #default="{ row }">
        ...
        <el-table-column prop="usedTime" label="激活时间" min-width="160">
          <template #default="{ row }">
    ...
    >
      <el-form :model="generateForm" label-width="80px" class="dialog-form">
        ...
        <p class="form-tip">每个商户仅保留一个未使用的注册码；若已有未使用码将直接返回原码。</p>
      </el-form>
      <template #footer>
    ...
    >
      <div class="generated-code-box">
        <p class="label">请将以下注册码交给商户，用于分控平台安装激活：</p>
        <div class="code-list">
          <div class="code-item">
        ...
        <p class="tip">注册码一次性使用，分控安装成功后即失效，请妥善保管！</p>
      </div>
      <template #footer>
...
<script setup>
/**
 * 分控安装注册码管理（仅总控）：为商户签发 BEND-INSTALL-* 码，供分控安装器激活。
 */
...
const handleGenerate = async () => {
  ...
  try {
    const res = await registrationCodeApi.generateInstall({
...
const handleBatchDelete = async () => {
  ...
  const message = usedCount > 0
    ? `选中了 ${selectedCodes.value.length} 个注册码，其中 ${usedCount} 个已激活将无法删除。确定删除 ${unusedCodes.length} 个未使用的注册码吗？`
