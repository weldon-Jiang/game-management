<template>
  <div class="page-container" v-loading="loading">
    ...
    <div v-if="task" class="content-card">
      <div class="overview-card">
        <div class="overview-grid">
          ...
          <div class="overview-item">
            ...
            <span>{{ task.streamingAccountName || task.streamingAccountId || '-' }}</span>
          </div>
          <div class="overview-item">
            <span class="label">执行 Agent</span>
            <span>{{ task.targetAgentName || task.targetAgentId || '未分配' }}</span>
          </div>
          <div class="overview-item">
            <span class="label">游戏操作</span>
            <span>{{ task.gameActionType ? getGameActionTypeText(task.gameActionType) : '-' }}</span>
          </div>
          <div class="overview-item">
        ...
        <el-alert
          v-if="lastProgressMessage"
          type="info"
          :closable="false"
          :title="`最新进度：${lastProgressMessage}`"
        ...
        <el-alert
          ...
          type="error"
          :closable="false"
          :title="`会话错误：${session.errorMessage}`"
          class="error-alert"
        />
      </div>

      <SessionPhaseStepper :phase="task.sessionPhase || session?.phase || 'opening'" />
      ...
      <TaskEventTimeline class="event-section" :events="events" />
    </div>
  </div>
</template>
...
const handleWindow = async (action) => {
  if (action === 'show') await taskApi.showWindow(taskId.value)
  else if (action === 'hide') await taskApi.hideWindow(taskId.value)
  else await taskApi.focusWindow(taskId.value)
  ElMessage.success('已发送窗口指令')
}
