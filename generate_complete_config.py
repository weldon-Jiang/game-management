...
# Pattern to match schema = [...] blocks with exact 15 elements
...
for match in matches:
    ...
    if numbers and len(numbers) >= 15:
        # Convert to integer list with exactly 15 elements
        schema = list(map(int, numbers[:15]))
        schemas.append(schema)

print(f"Found {len(schemas)} complete scene templates")

# Now generate the Python config file
...
for scene_id in sorted(scene_groups.keys()):
    desc = scene_descriptions.get(str(scene_id), f"场景{scene_id}")
    config_content += f"    {scene_id}: \"{desc}\",\n"
