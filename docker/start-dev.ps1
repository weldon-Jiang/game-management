...
# 开发环境 (dev) 启动 / 打包部署脚本
# 用法：
#   ./start-dev.ps1                              # 全量打包部署 (profile=full，首次用)
#   ./start-dev.ps1 -Services backend            # 只重新打包部署 backend
#   ./start-dev.ps1 -Services backend,gateway    # 重新打包部署 backend + gateway
#   ./start-dev.ps1 -Services backend -NoBuild   # 只重启 backend，不重新构建
#   ./start-dev.ps1 -Profile app                 # 只起应用层(前端+后端+网关)
#   ./start-dev.ps1 -Profile core                # 仅核心服务(Redis+后端+网关)
# 兼容旧写法：
#   ./start-dev.ps1 full backend                 # 位置参数：profile + 单服务
# ============================================
param(
    [string]$Profile = 'full',
    [string[]]$Services = @(),
