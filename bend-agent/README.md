...
### 步骤二：Xbox 串流连接（xsrp / GSSV 云端）

**入口**：`automation/step2_router.py` → `automation/step2_xsrp.py` → `xbox/step2_xsrp_connect.py`

**功能**：GSSV 云端主机发现、租约、play/WebRTC 握手；对齐 `streaming/xsrp.py` 的 OpenStreaming GSSV/WebRTC 段（Python aiortc 实现，非 SmartGlass LAN 主链路）。

**流程**：
1. Step1 写入的 GSSV/Xbox Token 校验（`streaming_credentials.attach_streaming_credentials`）
2. GSSV 云端发现与主机匹配（`XboxHostMatcher` + `connect_xsrp_cloud`）
3. play 会话 + WebRTC SDP 交换
4. Step2 内联触发 Step3 初始化（首帧/输入通道就绪后 Step2 完成）

**相关模块**：
- `xbox/xsrp_cloud_connect.py`：云端 play/WebRTC 连接
- `xbox/console_lease.py`：serverId 租约
- `gssv/`：GSSV API 客户端

**说明**：SmartGlass UDP（`xbox/xbox_discovery.py`）仅用于 LAN 发现/唤醒兜底，**不是** Step2 串流传输协议。

### 步骤三：串流环境初始化（xsrp 栈）

**入口**：`auth/step3_router.py` → `automation/step3_xsrp.py`（显示/输入 helper：`step3_display_helpers.py`）

**功能**：在 Step2 WebRTC 会话就绪后，建立 SDL 窗口、WebRTC 帧捕获与 DataChannel 输入，供 Step4 截图与自动化按键。

**流程**：
1. 绑定 `XsrpFrameCapture`（WebRTC 视频帧）
2. 初始化 SDL 串流窗口（`step3_display_helpers`）
3. 建立 xsrp 输入通道（`ControllerProtocol` / DataChannel）
4. 首帧与 input ready 校验；可选 idle keepalive

**输出**（写入 `AgentTaskContext`）：
- `context.frame_capture`：WebRTC 帧捕获
- `context._controller_protocol`：自动化/人工手柄通道
- `context._streaming_stack`：`"xsrp"`
- SDL 窗口与 display 相关字段（供 Step4 / task_control 显隐）
