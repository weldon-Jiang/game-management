"""
Step2 串流握手路由
================
按 account_platform 分发至 Xbox(xsrp GSSV) 或 PlayStation 实现。
"""
from typing import Callable

from ...task.task_context import AgentTaskContext, Step2Result
from ..platform_util import account_platform


async def step2_execute_streaming(
    context: AgentTaskContext,
    check_cancel: Callable[[], bool],
    report_progress: Callable,
) -> Step2Result:
    """Step2 统一入口：按平台分发。"""
    if account_platform(context) == "playstation":
        from .playstation_streaming import step2_execute_playstation_streaming
        return await step2_execute_playstation_streaming(
            context, check_cancel, report_progress
        )
    # Xbox 统一走 xblive/xsrp 云端 GSSV 串流
    from .xsrp_streaming import step2_execute_xsrp_streaming
    return await step2_execute_xsrp_streaming(context, check_cancel, report_progress)
